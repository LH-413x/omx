...
Harness::Harness(std::string uri)
    : mInitCheck(NO_INIT) {
    mInitCheck = initOMX();
    mUri = uri;
    createMaper();
...
*/
static sp<MediaSource> CreateSourceForMUri(const std::string & uri,const std::string & mime) {
    //const char *url = GetURLForMime(mime);

    sp<IMediaExtractor> extractor = CreateExtractorFromURI(uri.data());

    if (extractor == NULL) {
        return NULL;
    }
    else{
      ALOGI("get extractor from CreateExtractorFromURI");
    }
    ...
    for (size_t i = 0; i < extractor->countTracks(); ++i) {
        ...
        if (!strcasecmp(mime.data(), trackMime)) {
            ALOGI("start CreateMediaSourceFromIMediaSource");
            return CreateMediaSourceFromIMediaSource(extractor->getTrack(i));
        }
    }
    ALOGI("%s fail to CreateSourceForMUri",uri.data());
    return NULL;
}
...
status_t Harness::testSeek(
        const string componentName, const string componentRole) {
    bool isEncoder =
        !strncmp(componentRole.data(), "audio_encoder.", 14)
        || !strncmp(componentRole.data(), "video_encoder.", 14);
    ALOGI("start testSeek");
    if (isEncoder) {
        // Not testing seek behaviour for encoders.

        ALOGE("  * Not testing seek functionality for encoders.\n");
        return OK;
    }

    //const char *mime = GetMimeFromComponentRole(componentRole.data());

    string mime=mRoleMapMime[componentRole];
    if (!mime.data()) {
        ALOGE("  * Cannot perform seek test with this componentRole (%s)\n",
               componentRole.data());

        return OK;
    }
    else{
      ALOGI("mime: %s", mime.data());
    }

    sp<MediaSource> source = CreateSourceForMUri(mUri, mime);

    if (source == NULL) {
        ALOGE("  * Unable to open test content for type '%s', "
               "skipping test of componentRole %s\n",
               mime.data(), componentRole.data());

        return OK;
    }
    else{
      ALOGI("create sp<MediaSource> source success");
    }

    sp<MediaSource> seekSource = CreateSourceForMUri(mUri, mime);
    ...
    sp<MediaSource> codec = SimpleDecodingSource::Create(
            source, 0 /* flags */, NULL /* nativeWindow */, componentName.data());

    if(codec == NULL){
      ALOGE("fail to create codec");
      return UNKNOWN_ERROR;
    }
    ...
    int64_t durationUs;
    source->getFormat()->findInt64(kKeyDuration, &durationUs);

    ALOGI("stream duration is %lld us (%.2f secs)",
         durationUs, durationUs / 1E6);

    static const int32_t kNumIterations = 50;
    ...
    for (int32_t i = 0; i < kNumIterations; ++i) {
        ...
        if ((i == 1) || (i > 0 && r < 0.5)) {
            ...
            if (i == 0 || r < 0.55) {
                ...
                printf("requesting seek beyond EOF\n");
            } else {
                requestedSeekTimeUs =
                    (int64_t)(uniform_rand() * durationUs);

                printf("requesting seek to %lld us (%.2f secs)\n",
            ...
            printf("nearest keyframe is at %lld us (%.2f secs)\n",
...
status_t Harness::test(std::string componentName) {
  std::string componentRole=componentNameMapRole[componentName];
  ALOGI("testing %s [%s].", componentName.data(), componentRole.data());

  //status_t err1 = testStateTransitions(componentName.data(), componentRole.data());
  
  testSeek(componentName, componentRole);
  ALOGI("finish testSeek");
  return OK;
}

void Harness::createMaper(){
componentNameMapRole["OMX.google.aac.decoder"]=string("audio_decoder.aac");
componentNameMapRole["OMX.google.aac.encoder"]=string("audio_encoder.aac");
componentNameMapRole["OMX.google.amrnb.decoder"]=string("audio_decoder.amrnb");
componentNameMapRole["OMX.google.amrnb.encoder"]=string("audio_encoder.amrnb");
componentNameMapRole["OMX.google.amrwb.decoder"]=string("audio_decoder.amrwb");
componentNameMapRole["OMX.google.amrwb.encoder"]=string("audio_encoder.amrwb");
componentNameMapRole["OMX.google.flac.decoder"]=string("audio_decoder.flac");
componentNameMapRole["OMX.google.flac.encoder"]=string("audio_encoder.flac");
componentNameMapRole["OMX.google.g711.alaw.decoder"]=string("audio_decoder.g711alaw");
componentNameMapRole["OMX.google.g711.mlaw.decoder"]=string("audio_decoder.g711mlaw");
componentNameMapRole["OMX.google.gsm.decoder"]=string("audio_decoder.gsm");
componentNameMapRole["OMX.google.h263.decoder"]=string("video_decoder.h263");
componentNameMapRole["OMX.google.h263.encoder"]=string("video_encoder.h263");
componentNameMapRole["OMX.google.h264.decoder"]=string("video_decoder.avc");
componentNameMapRole["OMX.google.h264.encoder"]=string("video_encoder.avc");
componentNameMapRole["OMX.google.hevc.decoder"]=string("video_decoder.hevc");
componentNameMapRole["OMX.google.mp3.decoder"]=string("audio_decoder.mp3");
componentNameMapRole["OMX.google.mpeg2.decoder"]=string("video_decoder.mpeg2");
componentNameMapRole["OMX.google.mpeg4.decoder"]=string("video_decoder.mpeg4");
componentNameMapRole["OMX.google.mpeg4.encoder"]=string("video_encoder.mpeg4");
componentNameMapRole["OMX.google.opus.decoder"]=string("audio_decoder.opus");
componentNameMapRole["OMX.google.raw.decoder"]=string("audio_decoder.raw");
componentNameMapRole["OMX.google.vorbis.decoder"]=string("audio_decoder.vorbis");
componentNameMapRole["OMX.google.vp8.decoder"]=string("video_decoder.vp8");
componentNameMapRole["OMX.google.vp8.encoder"]=string("video_encoder.vp8");
componentNameMapRole["OMX.google.vp9.decoder"]=string("video_decoder.vp9");
componentNameMapRole["OMX.google.vp9.encoder"]=string("video_encoder.vp9");
componentNameMapRole["OMX.qcom.video.decoder.avc"]=string("video_decoder.avc");
componentNameMapRole["OMX.qcom.video.decoder.avc.secure"]=string("video_decoder.avc");
componentNameMapRole["OMX.qcom.video.decoder.h263"]=string("video_decoder.h263");
componentNameMapRole["OMX.qcom.video.decoder.hevc"]=string("video_decoder.hevc");
componentNameMapRole["OMX.qcom.video.decoder.hevc.secure"]=string("video_decoder.hevc");
componentNameMapRole["OMX.qcom.video.decoder.mpeg4"]=string("video_decoder.mpeg4");
componentNameMapRole["OMX.qcom.video.decoder.vp8"]=string("video_decoder.vp8");
componentNameMapRole["OMX.qcom.video.decoder.vp9"]=string("video_decoder.vp9");
componentNameMapRole["OMX.qcom.video.decoder.vp9.secure"]=string("video_decoder.vp9");
componentNameMapRole["OMX.qcom.video.encoder.avc"]=string("video_encoder.avc");
componentNameMapRole["OMX.qcom.video.encoder.h263"]=string("video_encoder.h263");
componentNameMapRole["OMX.qcom.video.encoder.hevc"]=string("video_encoder.hevc");
componentNameMapRole["OMX.qcom.video.encoder.mpeg4"]=string("video_encoder.mpeg4");
componentNameMapRole["OMX.qcom.video.encoder.vp8"]=string("video_encoder.vp8");

mRoleMapMime["video_decoder.avc"]=string("video/avc");
mRoleMapMime["video_decoder.mpeg4"]=string("video/mp4v-es");
mRoleMapMime["video_decoder.h263"]=string("video/3gpp");
mRoleMapMime["video_decoder.vp8"]=string("video/x-vnd.on2.vp8");
mRoleMapMime["video_decoder.vp9"]=string("video/x-vnd.on2.vp9");
mRoleMapMime["audio_decoder.amr"]=string("audio/3gpp");
mRoleMapMime["audio_decoder.amrnb"]=string("audio/3gpp");
mRoleMapMime["audio_decoder.amrwb"]=string("audio/amr-wb");
mRoleMapMime["audio_decoder.aac"]=string("audio/mp4a-latm");
mRoleMapMime["audio_decoder.mp3"]=string("audio/mpeg");
mRoleMapMime["audio_decoder.vorbis"]=string("audio/vorbis");
mRoleMapMime["audio_decoder.opus"]=string("audio/opus");
mRoleMapMime["audio_decoder.flac"]=string("audio/flac");
mRoleMapMime["audio_decoder.g711alaw"]=string(MEDIA_MIMETYPE_AUDIO_G711_ALAW);
mRoleMapMime["audio_decoder.g711mlaw"]=string(MEDIA_MIMETYPE_AUDIO_G711_MLAW);
}
/*
status_t Harness::testAll() {
  ...
  for (List<IOMX::ComponentInfo>::iterator it = componentInfos.begin();
      ...
      ALOGE("componentName: %s",componentName);
      for (List<String8>::const_iterator role_it = info.mRoles.begin();
          ...
          ALOGE("componentRole: \t\t\t%s",componentRole);
      }
  }
  for (List<IOMX::ComponentInfo>::iterator it = componentInfos.begin();
       it != componentInfos.end(); ++it) {
